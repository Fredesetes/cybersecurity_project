<template>
  <q-page padding>
    <div class="row justify-between">
      <h5 class="column q-my-xs text-bold text-dark">Add Network</h5>
    </div>
              ...
              <div class="col-6">
                <div class="column">
                  <div>
                    ...
                    <q-input outlined dense v-model="network.bssid" placeholder="Enter BSSID" lazy-rules :rules="rules" />
                  </div>
                  <div>
...
export default {
  ...
  data() {
    return {
      network: {
        bssid: "", essid: "", privacy: "", cipher: "", auth: "", description: "", cracked: "",
  ...
  methods: {
    async onSubmit() {
      try {
        await this.$store.dispatch('addNetwork', this.network);

        this.$notify.create({
          message: `Network Added with Success!`,
          color: 'blue',
          icon: 'done'
        });
